...
import { AgentChatMessage, ChatMessage } from '../common/chatThreadServiceTypes.js';
...
export type AgentSubagentRunChangeEvent = Readonly<
	...
	| { parentId: string; generation: number; id: string; mailboxTarget: string }
>;
...
export type AgentSubagentWaitChild = Readonly<{ id: string; status: AgentSubagentStatus; completion: 'pending' | 'unread' | 'delivered_now' | 'already_delivered'; roleName?: string; roleDescription?: string; usage: null }>;
...
export interface IAgentSubagentService {
	...
	hasPendingRequired(parentId: string, generation?: number): boolean;
	waitForRequired(parentId: string, generation?: number): Promise<void>;
	...
	ackParentMailbox(parentId: string, generation: number, claim: AgentSubagentParentMailboxClaim): boolean;
...
export class AgentSubagentService extends Disposable implements IAgentSubagentService {
	...
	async spawn(parentId: string, message: string, parent: AgentRuntimeTurnSnapshot, agentType?: string, admittedRoles?: CustomAgentCatalog, admittedSettingsState?: IVoidSettingsService['state'], admittedSettingsOfProvider?: SettingsOfProvider, generation = 0, parentTools?: AgentSubagentToolSnapshot, broker?: AgentSubagentToolBroker, model?: string, reasoningEffort?: string, forkTurns: 'none' | 'all' | number = 'none', forkHistory: readonly ChatMessage[] = [], background = false) {
		return this.spawnWithin(parentId, message, parent, agentType, admittedRoles, admittedSettingsState, admittedSettingsOfProvider, generation, undefined, parentTools, broker, model, reasoningEffort, forkTurns, forkHistory, background);
	}
	private async spawnWithin(parentId: string, message: string, parent: AgentRuntimeTurnSnapshot, agentType: string | undefined, admittedRoles: CustomAgentCatalog | undefined, admittedSettingsState: IVoidSettingsService['state'] | undefined, admittedSettingsOfProvider: SettingsOfProvider | undefined, generation: number, parentRun?: ChildRun, parentTools?: AgentSubagentToolSnapshot, broker?: AgentSubagentToolBroker, model?: string, reasoningEffort?: string, forkTurns: 'none' | 'all' | number = 'none', sourceHistory: readonly ChatMessage[] = [], background = false) {
		...
		const admissionWork = async () => { try {
			...
			const selectedModel = model ?? role?.model; const selectedEffort = reasoningEffort ?? role?.modelReasoningEffort;
			const roleModel = selectedModel || selectedEffort ? this.roleRuntimeModel(parentModel, selectedModel, selectedEffort, capturedState) : parentModel;
	...
	/** Keep only complete logical turns before the currently executing turn. */
	private forkHistory(source: readonly ChatMessage[], forkTurns: 'none' | 'all' | number): readonly ChatMessage[] {
		if (forkTurns === 'none' || source.length === 0) return Object.freeze([]);
		const starts: number[] = []; for (let i = 0; i < source.length; i++) if (source[i].role === 'user') starts.push(i);
		if (starts.length < 2) return Object.freeze([]);
		const completeEnd = starts[starts.length - 1];
		const completeStarts = starts.slice(0, -1); const first = forkTurns === 'all' ? 0 : Math.max(0, completeStarts.length - forkTurns);
		return Object.freeze(deepClone(source.slice(completeStarts[first], completeEnd)));
	}
	...
	private async run(run: ChildRun, message: string): Promise<void> {
		try {
			...
			const history: ChatMessage[] = [...deepClone(run.initialHistory), { role: 'user', content: message, displayContent: message, selections: [], state: { stagingSelections: [], isBeingEdited: false } }];
			for (let turn = 0; !run.cancellation.token.isCancellationRequested; turn++) {
				for (const queued of this.takeChildMailbox(run)) history.push(queued);
			...
			if (!responseTools.length) {
				// A message accepted while this provider request was in flight must reach a
				// model boundary before the child can become terminal.
				if (this.hasPendingRequiredFor(run.parentId, run.generation, run.id)) {
					await this.yieldForChildren(run);
					await this.waitForRequiredFor(run.parentId, run.generation, run.id);
					this.setSchedulerActivity(run, false, 'ready_to_resume');
					await this.reacquire(run);
					if (!this.isCurrent(run)) { this.settle(run, 'cancelled', 'Child cancelled or owner changed.'); return; }
					continue;
				}
				if (run.inbox.length || this.hasUnreadCompletionFor(run.parentId, run.generation, run.id)) continue;
				this.settle(run, 'completed', responseText.slice(0, this.groupBudget(run).maxChildSummaryChars)); return;
			}
			history.push({ role: 'assistant', displayContent: responseText, reasoning: '', anthropicReasoning: null, toolBatch: { version: 1 as const, batchId: childBatchId!, calls: responseTools.map(tool => ({ ...tool, isDone: true, doneParams: [] })) } });
			...
			if (isAgentSubagentControlName(tool.name)) {
				try {
					...
					let result: object;
					if (control.name === 'spawn_agent') result = await this.spawnWithin(run.parentId, control.message, run.snapshot, control.agentType, run.admittedRoles, run.admittedSettingsState, run.settingsOfProvider, run.generation, run, run.parentTools, run.broker, control.model, control.reasoningEffort, control.forkTurns, history, control.background);
					...
					else if (control.name === 'list_agents') result = this.listFor(run.parentId, run.id, control.target, run.generation);
					else if (control.name === 'send_message') result = this.sendMessageFor(run.parentId, run.id, control.target, control.message, run.generation);
	...
	private normalizedReport(value: string): string { return value.replace(/\r\n?/g, '\n').trim(); }
	private reportKey(target: string, value: string): string { return JSON.stringify([target, this.normalizedReport(value)]); }
	private rememberReport(group: ChildGroup, sourceId: string, target: string, message: string, sequence: number): void {
		let reports = group.reportedMessages.get(sourceId); if (!reports) { reports = new Map(); group.reportedMessages.set(sourceId, reports); }
		const normalized = this.reportKey(target, message); if (reports.has(normalized)) return;
		reports.set(normalized, Object.freeze({ sequence, target }));
	}
	private settle(run: ChildRun, status: Exclude<AgentSubagentStatus, 'queued' | 'running'>, summary: string): boolean {
		...
		if (group?.generation === run.generation) {
			...
			const reportTarget = run.parentRunId ?? run.parentId;
			const reported = group.reportedMessages.get(run.id)?.get(this.reportKey(reportTarget, summary));
			const reportedMessageSequence = reported?.sequence;
			group.reportedMessages.delete(run.id);
			group.completions.push(Object.freeze({ sequence: ++group.eventSequence, id: run.id, ...(run.parentRunId ? { parentRunId: run.parentRunId } : {}), generation: run.generation, status, summary: retainedSummary.slice(0, AGENT_SUBAGENT_MAX_MESSAGE_CHARS), ...(resultTruncated ? { resultTruncated: true as const } : {}), completedAt: run.settledAt, delivered: false, background: run.background, ...(reportedMessageSequence === undefined ? {} : { reportedMessageSequence }) }));
		}
	...
	private hasPendingRequiredFor(parentId: string, generation: number, directParentRunId: string | undefined): boolean {
		const group = this.groups.get(parentId); return !!group && group.generation === generation && !group.cancellation && group.runs.some(run => run.parentRunId === directParentRunId && !run.background && (run.lifecycle.status === 'queued' || run.lifecycle.status === 'running'));
	}
	hasPendingRequired(parentId: string, generation = 0): boolean { return this.hasPendingRequiredFor(parentId, generation, undefined); }
	private hasUnreadCompletionFor(parentId: string, generation: number, directParentRunId: string | undefined): boolean { const group = this.groups.get(parentId); return !!group && group.generation === generation && group.completions.some(item => item.parentRunId === directParentRunId && !item.background && !item.delivered); }
	private async waitForRequiredFor(parentId: string, generation: number, directParentRunId: string | undefined): Promise<void> {
		const hasCoordinationMessage = () => {
			const group = this.groups.get(parentId); if (!group || group.generation !== generation) return false;
			return directParentRunId ? !!group.runs.find(run => run.id === directParentRunId)?.inbox.length : group.parentInbox.length > 0;
		};
		if (!this.hasPendingRequiredFor(parentId, generation, directParentRunId) || hasCoordinationMessage()) return;
		await new Promise<void>(resolve => {
			let settled = false;
			const finish = () => { if (settled) return; settled = true; listener.dispose(); resolve(); };
			const listener = this.onDidChangeRun(event => { if (event.parentId === parentId && event.generation === generation && ((!this.hasPendingRequiredFor(parentId, generation, directParentRunId)) || (('mailboxTarget' in event) && event.mailboxTarget === (directParentRunId ?? parentId)))) finish(); });
			if (!this.hasPendingRequiredFor(parentId, generation, directParentRunId) || hasCoordinationMessage()) finish();
		});
	}
	waitForRequired(parentId: string, generation = 0): Promise<void> { return this.waitForRequiredFor(parentId, generation, undefined); }
	private completionEvent(item: AgentSubagentCompletion): AgentChatMessage { return Object.freeze({ role: 'agent', sourceId: item.id, kind: 'completion', sequence: item.sequence, ...(item.reportedMessageSequence === undefined ? {} : { relatedSequence: item.reportedMessageSequence }), status: item.status, content: item.reportedMessageSequence === undefined ? item.summary : '', createdAt: item.completedAt }); }
	private takeChildMailbox(run: ChildRun): AgentChatMessage[] {
		const group = this.groups.get(run.parentId); if (!group || group.generation !== run.generation) return [];
		const completions = group.completions.filter(item => item.parentRunId === run.id && !item.background && !item.delivered);
		for (const item of completions) { const index = group.completions.indexOf(item); group.completions[index] = Object.freeze({ ...item, delivered: true }); }
		return [...run.inbox.splice(0), ...completions.map(item => this.completionEvent(item))].sort((a, b) => a.sequence - b.sequence);
	}
	...
	private async waitFor(parentId: string, directParentRunId: string | undefined, timeoutMs: number, targets: readonly string[] | undefined, generation: number): Promise<AgentSubagentWaitResult> {
		...
		const modelReceipt = (receipt: AgentSubagentReceipt, completion: AgentSubagentCompletion | undefined): AgentSubagentReceipt => completion?.reportedMessageSequence === undefined ? receipt : Object.freeze({ ...receipt, summary: '', reportedMessageSequence: completion.reportedMessageSequence });
		const newlyDelivered = unread.map(item => Object.freeze({ id: item.id, status: item.status, summary: item.reportedMessageSequence === undefined ? item.summary : '', ...(item.resultTruncated ? { resultTruncated: true as const } : {}), ...(item.reportedMessageSequence === undefined ? {} : { reportedMessageSequence: item.reportedMessageSequence }), usage: null }));
		...
		const allReceipts = selected.map(run => { const receipt = run.lifecycle.inspectReceipt(); return receipt ? modelReceipt(receipt, group.completions.find(item => item.id === run.id)) : undefined; }).filter((receipt): receipt is AgentSubagentReceipt => !!receipt); timedOut = timedOut && newlyDelivered.length === 0;
		...
		return { ...legacy, children: Object.freeze(selected.map(run => { const completion = group.completions.find(item => item.id === run.id); return Object.freeze({ id: run.id, status: run.lifecycle.status, completion: deliveredNow.has(run.id) ? 'delivered_now' as const : !completion ? 'pending' as const : completion.delivered ? 'already_delivered' as const : 'unread' as const, ...(run.role ? { roleName: run.role.name, roleDescription: run.role.description } : {}), usage: null }); })), ...(allReceipts.length === 1 ? { receipt: allReceipts[0] } : { receipts: Object.freeze(allReceipts) }), budget: this.budget(group), deliverSummary: newlyDelivered.length > 0, timedOut };
	}
	list(parentId: string, target?: string, generation = 0): object { return this.listFor(parentId, undefined, target, generation); }
	private listFor(parentId: string, _callerRunId: string | undefined, target: string | undefined, generation: number): object {
		const group = this.groups.get(parentId); if (!group || group.generation !== generation) throw new Error('agent_child_not_found');
		if (target && target !== parentId && !group.runs.some(run => run.id === target)) throw new Error('agent_target_not_found');
		const candidates = target === parentId || !target ? group.runs : group.runs.filter(run => run.id === target || this.isDescendantOf(group, run, target)); const included = candidates.slice(0, 32); let remainingChars = 16_000;
		const agents = included.map(run => { const completion = group.completions.find(item => item.id === run.id); const summary = completion?.summary.slice(0, Math.max(0, Math.min(2000, remainingChars))); remainingChars -= summary?.length ?? 0; return Object.freeze({ id: run.id, parent: run.parentRunId ?? parentId, depth: run.depth, status: run.lifecycle.status, completion: !completion ? 'pending' : completion.delivered ? 'already_delivered' : 'unread', ...(summary ? { summary, ...(summary.length < completion!.summary.length || completion!.resultTruncated ? { resultTruncated: true } : {}) } : {}), ...(run.role ? { role: run.role.name } : {}) }); });
		return Object.freeze({ root: Object.freeze({ id: parentId, target: parentId, status: group.cancellation ? 'cancelled' : 'active' }), agents: Object.freeze(agents), omitted: Math.max(0, candidates.length - included.length) });
	}
	private isDescendantOf(group: ChildGroup, run: ChildRun, ancestor: string): boolean { let parent = run.parentRunId; while (parent) { if (parent === ancestor) return true; parent = group.runs.find(candidate => candidate.id === parent)?.parentRunId; } return false; }
	sendMessage(parentId: string, target: string, message: string, generation = 0): object { return this.sendMessageFor(parentId, undefined, target, message, generation); }
	private sendMessageFor(parentId: string, callerRunId: string | undefined, target: string, message: string, generation: number): object {
		const group = this.groups.get(parentId); if (!group || group.generation !== generation || group.cancellation) throw new Error('agent_child_not_found');
		if (!message.trim() || message.length > AGENT_SUBAGENT_MAX_MESSAGE_CHARS) throw new Error('send_message_invalid_params');
		if (target === parentId) {
			if (!callerRunId) throw new Error('agent_message_self');
			if (group.parentInbox.length >= 32 || group.parentInbox.reduce((total, item) => total + item.content.length, 0) + message.length > 64 * 1024) throw new Error('agent_message_queue_full');
			const sequence = ++group.eventSequence;
			group.parentInbox.push(Object.freeze({ role: 'agent', sourceId: callerRunId, kind: 'message', sequence, content: message, createdAt: Date.now() }));
			this.rememberReport(group, callerRunId, target, message, sequence);
			this._onDidChangeRun.fire({ parentId, generation, id: callerRunId, mailboxTarget: parentId }); return Object.freeze({ target, status: 'queued' });
		}
		const run = group.runs.find(candidate => candidate.id === target); if (!run) throw new Error('agent_target_not_found');
		if (run.id === callerRunId) throw new Error('agent_message_self');
		if (run.lifecycle.status !== 'queued' && run.lifecycle.status !== 'running') throw new Error('agent_target_terminal');
		const sourceId = callerRunId ?? parentId; const sequence = ++group.eventSequence;
		if (run.inbox.length >= 32 || run.inbox.reduce((total, item) => total + item.content.length, 0) + message.length > 64 * 1024) throw new Error('agent_message_queue_full');
		run.inbox.push(Object.freeze({ role: 'agent', sourceId, kind: 'message', sequence, content: message, createdAt: Date.now() }));
		if (callerRunId) this.rememberReport(group, callerRunId, target, message, sequence);
		this._onDidChangeRun.fire({ parentId, generation, id: callerRunId ?? parentId, mailboxTarget: run.id });
		return Object.freeze({ target, status: 'queued' });
	}
	peekParentMailbox(parentId: string, generation = 0): AgentSubagentParentMailboxClaim {
		const group = this.groups.get(parentId); if (!group || group.generation !== generation || group.cancellation) return Object.freeze({ events: Object.freeze([]), parentMessageSequences: Object.freeze([]), completionSequences: Object.freeze([]) });
		const unread = group.completions.filter(item => !item.parentRunId && !item.background && !item.delivered);
		const completions = unread.map(item => this.completionEvent(item));
		return Object.freeze({ events: Object.freeze([...group.parentInbox, ...completions].sort((a, b) => a.sequence - b.sequence)), parentMessageSequences: Object.freeze(group.parentInbox.map(item => item.sequence)), completionSequences: Object.freeze(unread.map(item => item.sequence)) });
	}
	ackParentMailbox(parentId: string, generation: number, claim: AgentSubagentParentMailboxClaim): boolean {
		const group = this.groups.get(parentId); if (!group || group.generation !== generation || group.cancellation) return false;
		const parentSequences = new Set(claim.parentMessageSequences); if ([...parentSequences].some(sequence => !group.parentInbox.some(item => item.sequence === sequence))) return false;
		group.parentInbox.splice(0, group.parentInbox.length, ...group.parentInbox.filter(item => !parentSequences.has(item.sequence))); const sequences = new Set(claim.completionSequences);
		for (let index = 0; index < group.completions.length; index++) if (sequences.has(group.completions[index].sequence) && !group.completions[index].delivered) group.completions[index] = Object.freeze({ ...group.completions[index], delivered: true });
		return true;
	}
	...
	private interruptFor(parentId: string, callerRunId: string | undefined, target: string, generation: number) { const group = this.groups.get(parentId); const run = group?.generation === generation ? group.runs.find(candidate => candidate.id === target) : undefined; if (!run) throw new Error('agent_target_not_found'); if (run.lifecycle.status !== 'queued' && run.lifecycle.status !== 'running') throw new Error('agent_target_terminal'); if (callerRunId && (target === callerRunId || this.isDescendantOf(group!, group!.runs.find(candidate => candidate.id === callerRunId)!, target))) throw new Error('agent_interrupt_ancestor_or_self'); this.cancelRunTree(group!, run, 'Child cancelled by agent.'); return { id: run.id, status: run.lifecycle.status, receipt: run.lifecycle.inspectReceipt() }; }
