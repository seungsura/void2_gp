...
export const agentSubagentControlTools: readonly InternalToolInfo[] = Object.freeze([
	agentSubagentControlTool({ name: 'spawn_agent', description: 'Start a child asynchronously, then continue useful main-agent work. By default the child is required for the current answer: your final candidate is held until it finishes so you can synthesize its terminal result. Set background=true only for independent work that must not delay the current answer. Optional agent_type selects an admitted role. Explicit model and reasoning_effort override that role and the parent; omitted values use role then parent settings. fork_turns defaults to none and accepts none, all, or a positive decimal count of completed logical turns. Generic children remain read-only. The child has no OS sandbox.', params: { message: { description: 'The bounded delegated task.' }, agent_type: { description: 'Optional exact admitted custom-agent identity.' }, model: { description: 'Optional model available on the frozen parent provider.' }, reasoning_effort: { description: 'Optional effort supported by the effective model.' }, fork_turns: { description: 'Optional none, all, or positive decimal number of completed logical turns; defaults to none.' }, background: { description: 'Optional. False by default means required for the current answer; true means independent background work.' } }, schema: agentSubagentToolSchemas.spawn_agent }),
	agentSubagentControlTool({ name: 'wait_agent', description: 'Wait for selected direct children, or all direct children when targets is omitted. It wakes immediately for completion, coordination, or a user Steer, with a one-hour maximum. Continue useful main work after spawning, then normally omit timeout_ms and wait once instead of repeatedly polling. Terminal receipts remain inspectable on repeated waits; deliverSummary says whether this call delivered a newly queued completion.', params: { timeout_ms: { description: 'Optional wait time in milliseconds, 0 through 3600000. Omit it for an ordinary wait.' }, targets: { description: 'Optional array of distinct direct-child ids within the protocol limit; XML tag body is a JSON array such as ["id1","id2"].' } }, schema: agentSubagentToolSchemas.wait_agent }),
	agentSubagentControlTool({ name: 'list_agents', description: 'Inspect the current same-root agent tree, status, and bounded terminal summary without consuming completion delivery. Optional target selects one visible agent and its descendants.', params: { target: { description: 'Optional same-root agent id.' } }, schema: agentSubagentToolSchemas.list_agents }),
	agentSubagentControlTool({ name: 'send_message', description: 'Queue an intermediate coordination message for an active same-root parent, peer, or child. A child final answer is reported automatically; do not send the same final answer with send_message. Delivery occurs at the target model\'s next safe boundary and wakes an active wait. It never resumes a terminal child.', params: { target: { description: 'The active same-root agent id.' }, message: { description: 'The intermediate coordination message.' } }, schema: agentSubagentToolSchemas.send_message }),
	agentSubagentControlTool({ name: 'interrupt_agent', description: 'Cancel an active same-root descendant or peer by id. A child cannot cancel itself or an ancestor.', params: { target: { description: 'The same-root target id.' } }, schema: agentSubagentToolSchemas.interrupt_agent }),
]);
...
export const availableTools = (chatMode: ChatMode | null, mcpTools: InternalToolInfo[] | undefined, toolExecutionProfile: ToolExecutionProfile = 'default-parent', agentDelegationAllowed = false, frozenToolSnapshot?: AgentSubagentToolSnapshot) => {
	...
	const controlTools = chatMode !== 'agent' ? [] : toolExecutionProfile === 'default-parent'
		? (agentDelegationAllowed ? agentSubagentControlTools : [])
		: agentSubagentControlTools.filter(tool => tool.name !== 'spawn_agent' || agentDelegationAllowed)
