...
const buildSkippedToolCardRuntime = async (sidebarPath: string) => {
	...
	fs.writeFileSync(entry, `
		...
		import { createRoot } from 'react-dom/client';
		import { flushSync } from 'react-dom';
		import { isPureAgentWaitTimeoutResult } from ${JSON.stringify(path.join(sourceRoot, 'src', 'vs', 'workbench', 'contrib', 'void', 'common', 'agentSubagents.ts'))};
		...
		const rows = [
			...
			{ role: 'tool', type: 'success', name: 'wait_agent', result: { timedOut: true, deliverSummary: false, children: [{ id: 'child', status: 'running', completion: 'pending', usage: null }], receipts: [], budget: {} }, content: 'persisted pure timeout', id: 'wait-timeout', rawParams: {}, params: { timeout_ms: 3600000 }, mcpServerName: undefined },
			{ role: 'tool', type: 'success', name: 'wait_agent', result: { timedOut: false, deliverSummary: true, children: [{ id: 'child', status: 'completed', completion: 'delivered_now', usage: null }], receipt: { id: 'child', status: 'completed', summary: 'done', usage: null }, budget: {} }, content: 'persisted completion', id: 'wait-complete', rawParams: {}, params: {}, mcpServerName: undefined },
			{ role: 'tool', type: 'tool_error', name: 'wait_agent', result: 'wait failed', content: 'wait failed', id: 'wait-error', rawParams: {}, params: {}, mcpServerName: undefined },
		] as const;
		let root: ReturnType<typeof createRoot> | undefined;
		const Harness = () => <>{rows.map(row => { const early = renderEarlyToolCard(row as any); return <React.Fragment key={row.id}>{early === null ? null : early ?? <article data-tool-card="typed" data-tool-id={row.id}>typed route</article>}</React.Fragment>; })}</>;
...
const buildMountedSidebarRuntime = async (sidebarPath: string) => {
	...
	fs.writeFileSync(entry, `
		...
		const quietWait: any = { role: 'tool', type: 'success', name: 'wait_agent', id: 'quiet-wait', params: {}, rawParams: {}, result: { timedOut: true, deliverSummary: false, children: [{ id: 'child-root', status: 'running', completion: 'pending' }], receipts: [] }, content: 'PURE_TIMEOUT_MUST_NOT_RENDER' };
		...
		const currentThread: any = { id: 'thread-current', messages: [receipt, quietWait], state: { stagingSelections: [], mountedInfo }, childActivities: { version: 1, records: [durable], omitted: 0, retentionSaturated: false } };
	...
	const modules: Record<string, string> = {
		...
		'../../../../common/agentSubagents.js': `export type AgentSubagentRunView = any; export type ChildActivitiesLedger = any; export type ChildActivityRecord = any; export type ChildToolApprovalView = any; export const isActiveChildRun = (run: any) => run.status === 'queued' || run.status === 'running'; export const isPureAgentWaitTimeoutResult = (value: any) => { if (!value || typeof value !== 'object' || Array.isArray(value) || value.timedOut !== true || value.deliverSummary !== false || Object.prototype.hasOwnProperty.call(value, 'receipt')) return false; if (value.receipts !== undefined && (!Array.isArray(value.receipts) || value.receipts.length !== 0)) return false; return Array.isArray(value.children) && value.children.length > 0 && value.children.every((child: any) => child && typeof child === 'object' && !Array.isArray(child) && child.completion === 'pending' && (child.status === 'queued' || child.status === 'running')); };`,
...
suite('Child Run live timing', function () {
	...
	test('refreshes the three views together while active and clears its only timer', async () => {
		...
		try {
			...
			assert.deepStrictEqual(await page.locator('[data-tool-card="typed"]').evaluateAll(cards => cards.map(card => card.getAttribute('data-tool-id'))), ['wait-complete', 'wait-error']);
			assert.deepStrictEqual(await page.evaluate(() => (window as any).__skippedToolCard.hasParams()), [false, false, true, true, true]); assert.deepStrictEqual(await page.evaluate(() => (window as any).__skippedToolCard.rawParamKinds()), ['object', 'object', 'object', 'object', 'object']); await page.evaluate(() => (window as any).__skippedToolCard.dispose());
	...
	test('keeps a durable child card beside its exact spawn receipt without changing transcript indices', async function () {
		...
		try {
			...
			assert.strictEqual((await page.locator('body').textContent())?.includes('PURE_TIMEOUT_MUST_NOT_RENDER'), false);
