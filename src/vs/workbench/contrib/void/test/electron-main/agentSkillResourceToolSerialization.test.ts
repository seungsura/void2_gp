...
import { anthropicTools, createAnthropicToolPreviewAccumulator, createGeminiToolCallAccumulator, geminiTools, openAITools } from '../../electron-main/llmMessage/sendLLMMessage.impl.js';
...
suite('Void selected Skill resource production serialization', () => {
	test('serializes no-marker native Agent authority through every production provider path', () => {
		...
		for (const names of emitted) for (const name of ['spawn_agent', 'wait_agent', 'list_agents', 'send_message', 'interrupt_agent']) assert.ok(names.includes(name));
	});
	test('serializes bounded wait_agent targets through every provider path and gates child controls by depth authority', () => {
		...
		for (const emittedChild of [openAITools('agent', undefined, 'read-only-child', true)!.map(tool => tool.function.name), anthropicTools('agent', undefined, 'read-only-child', true)!.map(tool => tool.name), geminiTools('agent', undefined, 'read-only-child', true)![0].functionDeclarations!.map(tool => tool.name)]) for (const name of ['spawn_agent', 'wait_agent', 'list_agents', 'send_message', 'interrupt_agent']) assert.ok(emittedChild.includes(name));
		assert.strictEqual(availableTools('agent', undefined, 'read-only-child')!.some(tool => tool.name === 'wait_agent'), true);
		assert.strictEqual(availableTools('agent', undefined, 'read-only-child')!.some(tool => tool.name === 'spawn_agent'), false);
	...
	test('serializes an immutable inherited parent profile through native and XML paths', () => {
		...
		for (const names of sets) { assert.ok(names.includes('write_file')); assert.ok(names.includes('captured_mcp')); assert.strictEqual(names.includes('live_other'), false); for (const control of ['spawn_agent', 'wait_agent', 'list_agents', 'send_message', 'interrupt_agent']) assert.ok(names.includes(control)); }
		...
		let capturedCall: any; const inheritedWrapper = extractXMLToolsWrapper(() => { }, params => { capturedCall = params.toolCalls?.[0]; }, 'agent', [{ name: 'live_replaced', description: 'Live only.', params: {} }], 'inherited-parent-write-child', true, snapshot);
		...
		let liveOnly: any; const liveOnlyWrapper = extractXMLToolsWrapper(() => { }, params => { liveOnly = params.toolCalls?.[0]; }, 'agent', [{ name: 'live_replaced', description: 'Live only.', params: {} }], 'inherited-parent-write-child', true, snapshot);
	...
	test('normalizes only valid XML wait fields before strict shared validation', () => {
		let tool: any; const wrapper = extractXMLToolsWrapper(() => { }, params => { tool = params.toolCalls?.[0]; }, 'agent', undefined, 'default-parent', true);
		...
		let malformed: any; const malformedWrapper = extractXMLToolsWrapper(() => { }, params => { malformed = params.toolCalls?.[0]; }, 'agent', undefined, 'default-parent', true);
	...
	test('normalizes only canonical XML page numbers for paginated tools', () => {
		...
		for (const malformed of ['01', '0', '2junk', '2e1', '1.5']) {
			let tool: any; const malformedWrapper = extractXMLToolsWrapper(() => { }, params => { tool = params.toolCalls?.[0]; }, 'agent', undefined);
		...
		let unsafe: any; const unsafeWrapper = extractXMLToolsWrapper(() => { }, params => { unsafe = params.toolCalls?.[0]; }, 'agent', undefined);
	...
	test('round-trips the shared flat registry through OpenAI and Anthropic', () => {
		...
		const preview = createAnthropicToolPreviewAccumulator();
		assert.strictEqual(preview.start(4, { type: 'tool_use', id: 'read-2', name: 'read_file' }), true);
		assert.strictEqual(preview.start(1, { type: 'tool_use', id: 'list-1', name: 'list_dir' }), true);
		assert.strictEqual(preview.appendJson(4, '{"uri":"b'), true);
		assert.strictEqual(preview.appendJson(1, '{"uri":"a'), true);
		assert.strictEqual(preview.appendJson(4, '"}'), true);
		assert.strictEqual(preview.appendJson(1, '"}'), true);
		assert.deepStrictEqual(preview.preview(), [
			{ id: 'list-1', name: 'list_dir', partialJson: '{"uri":"a"}' },
			{ id: 'read-2', name: 'read_file', partialJson: '{"uri":"b"}' },
		]);
		assert.strictEqual(preview.start(1, { type: 'tool_use', id: 'other', name: 'read_file' }), false);
		assert.strictEqual(preview.malformed, true);
	});
	...
	test('serializes the production Gemini declaration without numeric minLength', () => {
		...
		const retransmission = createGeminiToolCallAccumulator();
		assert.strictEqual(retransmission.consumeChunk([{ id: 'one', name: 'read_file', args: { uri: 'a' } }, { id: 'two', name: 'list_dir', args: { uri: 'b' } }]), true);
		assert.strictEqual(retransmission.consumeChunk([{ id: 'one', name: 'read_file', args: { uri: 'a' } }]), true);
		assert.deepStrictEqual(retransmission.calls().map(call => [call.id, call.name, call.params]), [['one', 'read_file', '{"uri":"a"}'], ['two', 'list_dir', '{"uri":"b"}']]);
		const duplicateInChunk = createGeminiToolCallAccumulator();
		assert.strictEqual(duplicateInChunk.consumeChunk([{ id: 'one', name: 'read_file', args: {} }, { id: 'one', name: 'read_file', args: {} }]), false);
		assert.strictEqual(duplicateInChunk.malformed, true);
		const idlessSameChunk = createGeminiToolCallAccumulator();
		assert.strictEqual(idlessSameChunk.consumeChunk([{ name: 'read_file', args: { uri: 'same' } }, { name: 'read_file', args: { uri: 'same' } }]), true);
		assert.strictEqual(idlessSameChunk.calls().length, 2); assert.notStrictEqual(idlessSameChunk.calls()[0].id, idlessSameChunk.calls()[1].id);
		assert.strictEqual(idlessSameChunk.consumeChunk([{ name: 'read_file', args: { uri: 'later' } }]), false);
		const conflictingId = createGeminiToolCallAccumulator();
		assert.strictEqual(conflictingId.consumeChunk([{ id: 'one', name: 'read_file', args: {} }]), true);
		assert.strictEqual(conflictingId.consumeChunk([{ id: 'one', name: 'list_dir', args: {} }]), false);
		assert.strictEqual(conflictingId.malformed, true);
	});
