...
suite('AGENTS instruction runtime paths', () => {
	...
	test('keeps one immutable instruction snapshot through retry and tool-loop sends', async () => {
		...
		const receiver = {
			...
			_agentSubagentService: { peekParentMailbox: () => ({ events: Object.freeze([]), parentMessageSequences: Object.freeze([]), completionSequences: Object.freeze([]) }), ackParentMailbox: () => true, hasPendingRequired: () => false, waitForRequired: async () => { } },
			_appendAgentMailboxEvents(...args: any[]) { return (ChatThreadService.prototype as any)._appendAgentMailboxEvents.apply(this, args); },
			_wakeAgentWaitForPendingSteer: () => false,
