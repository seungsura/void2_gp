...
suite('Void AgentSubagentService', () => {
	test('removes the exact empty sentinel from child continuation history, outbound request, and summary', async () => {
		const f = fixture({
			...
			send: (options, turn) => {
				queueMicrotask(() => options.onFinalMessage(turn === 1
					? { fullText: INTERNAL_EMPTY_MESSAGE_SENTINEL, fullReasoning: '', anthropicReasoning: null, toolCalls: [{ id: 'tool-1', name: 'ls_dir', rawParams: { uri: 'file:///workspace' } }, { id: 'tool-2', name: 'ls_dir', rawParams: { uri: 'file:///workspace' } }] }
		...
		assert.deepStrictEqual(outboundAssistant.tool_calls, [
			{ type: 'function', id: 'tool-1', function: { name: 'ls_dir', arguments: '{"uri":"file:///workspace"}' } },
			{ type: 'function', id: 'tool-2', function: { name: 'ls_dir', arguments: '{"uri":"file:///workspace"}' } },
		]);
		assert.deepStrictEqual(f.toolCalls.map(call => call.name), ['ls_dir', 'ls_dir']);
		const childHistory = converted[1].history as any[];
		const childDeclaration = childHistory.find(message => message.role === 'assistant' && message.toolBatch);
		assert.deepStrictEqual(childHistory.filter(message => message.role === 'tool').map(message => [message.id, message.batchId === childDeclaration.toolBatch.batchId, message.batchOrdinal]), [['tool-1', true, 0], ['tool-2', true, 1]]);
	...
	test('broker cancellation before write preparation settles without a mutation', async () => {
		...
		const write = parentTools.tools.find(tool => tool.name === 'write_file')!; const request: any = Object.freeze({ parentId: 'parent', generation: 4, childId: 'child', batchId: 'write-batch', batchOrdinal: 0, toolId: 'write', name: 'write_file', tool: write, rawParams: Object.freeze({ uri: 'file:///workspace/a', content: 'gamma' }), snapshotRevision: parentTools.revision, maxReadOutputTokens: 10, cancellationToken: { isCancellationRequested: false } });
	...
	test('broker rejects invalid frozen builtin params before publishing approval', async () => {
		...
		const broker = (ChatThreadService.prototype as any)._createAgentSubagentToolBroker.call(receiver, 'parent', authority);
		const write = parentTools.tools.find(tool => tool.name === 'write_file')!; const request: any = Object.freeze({ parentId: 'parent', generation: 4, childId: 'child', batchId: 'write-invalid-batch', batchOrdinal: 0, toolId: 'write-invalid', name: 'write_file', tool: write, rawParams: Object.freeze({}), snapshotRevision: parentTools.revision, maxReadOutputTokens: 10, cancellationToken: { isCancellationRequested: false } });
	...
	test('broker manually approves or rejects validated edit, terminal, and MCP calls exactly once', async () => {
		...
		bindManualApproval(receiver); const broker = (ChatThreadService.prototype as any)._createAgentSubagentToolBroker.call(receiver, 'parent', authority); const request = (name: string, toolId: string, rawParams: any) => Object.freeze({ parentId: 'parent', generation: 4, childId: 'child', batchId: `batch-${toolId}`, batchOrdinal: 0, toolId, name, tool: parentTools.tools.find(tool => tool.name === name)!, rawParams: Object.freeze(rawParams), snapshotRevision: parentTools.revision, maxReadOutputTokens: 10, cancellationToken: { isCancellationRequested: false } });
		...
		const approved = request('write_file', 'write-approved', { uri: 'file:///workspace/a', content: 'x' }); const approvedPending = broker.execute(approved); await Promise.resolve(); const approval = [...receiver._childToolApprovals.values()][0].view; assert.strictEqual(receiver.approveChildToolApproval({ ...approval.key, batchId: 'wrong-batch' }), false); assert.strictEqual(receiver.approveChildToolApproval({ ...approval.key, batchOrdinal: 1 }), false); assert.strictEqual(receiver.approveChildToolApproval(approval.key), true); assert.deepStrictEqual((await approvedPending).ok, true); assert.strictEqual(receiver.approveChildToolApproval(approval.key), false);
	...
	test('manual approval is frozen per turn, survives task switching, and cancellation or revocation removes it', async () => {
		...
		bindManualApproval(receiver); const broker = (ChatThreadService.prototype as any)._createAgentSubagentToolBroker.call(receiver, 'parent', authority); const tool = parentTools.tools.find(tool => tool.name === 'run_command')!; const request = (toolId: string): any => Object.freeze({ parentId: 'parent', generation: 4, childId: 'child', batchId: `batch-${toolId}`, batchOrdinal: 0, toolId, name: 'run_command', tool, rawParams: Object.freeze({ command: 'dir' }), snapshotRevision: parentTools.revision, maxReadOutputTokens: 10, cancellationToken: { isCancellationRequested: false } });
	...
	test('broker rejects concurrent and settled exact replay with one terminal side effect and interrupt', async () => {
		...
		const broker = (ChatThreadService.prototype as any)._createAgentSubagentToolBroker.call(receiver, 'parent', authority); const tool = parentTools.tools.find(tool => tool.name === 'run_command')!; const request = (batchId = 'batch-a', batchOrdinal = 0, raw: any = {}) => Object.freeze({ parentId: 'parent', generation: 4, childId: 'child', batchId, batchOrdinal, toolId: 'same-tool', name: 'run_command', tool, rawParams: Object.freeze(raw), snapshotRevision: parentTools.revision, maxReadOutputTokens: 10, cancellationToken: { isCancellationRequested: false } });
		...
		assert.deepStrictEqual(await broker.execute(request()), { ok: false, error: 'tool_replayed' }); assert.strictEqual(calls, 1); assert.strictEqual(interrupts, 1); assert.deepStrictEqual((await broker.execute(request('batch-a', 1))).ok, true); assert.deepStrictEqual((await broker.execute(request('batch-b'))).ok, true); assert.strictEqual(calls, 3);
	});
	...
	test('broker terminal cancellation interrupts exactly once and acknowledges only after its result settles', async () => {
		...
		const broker = (ChatThreadService.prototype as any)._createAgentSubagentToolBroker.call(receiver, 'parent', authority); const tool = parentTools.tools.find(tool => tool.name === 'run_command')!; const request: any = Object.freeze({ parentId: 'parent', generation: 4, childId: 'child', batchId: 'terminal-batch', batchOrdinal: 0, toolId: 'terminal', name: 'run_command', tool, rawParams: Object.freeze({ command: 'dir' }), snapshotRevision: parentTools.revision, maxReadOutputTokens: 10, cancellationToken: { isCancellationRequested: false } });
	...
	test('marker-free real ChatThread admission routes an advertised custom role through spawn, wait, and its frozen broker', async () => {
		...
		receiver._pendingBroker = () => ({
			authorizeDirectHistoryAppend: async () => ({ ok: true, snapshot: brokerSnapshot, value: { leaseId: `direct-lease-${++directAuthorizations}`, pendingInputId: 'direct-input-1', selectionsFingerprint: 'fixture-selections' } }),
			verifyDirectHistoryAndRelease: async () => { directVerifications++; return { ok: true, snapshot: brokerSnapshot, value: { kind: 'exact', envelopeRaw: '{}' } }; },
			abandonDirectHistoryAppend: async () => ({ ok: true, snapshot: brokerSnapshot, value: undefined }),
		});
		receiver._awaitThreadStorageWrites = async () => true;
		receiver._threadFromBrokerHistoryInspection = (threadId: string) => receiver.state.allThreads[threadId];
	...
	test('broker cancellation acknowledgement releases a terminal writer even when execute never settles', async () => {
		...
		const f = fixture({ customCatalog: roles, broker: { execute: () => never, cancel: () => ack }, send: options => { sends++; queueMicrotask(() => sends === 1 ? options.onFinalMessage({ fullText: 'write', fullReasoning: '', anthropicReasoning: null, toolCalls: [{ id: 'write', name: 'captured_mcp', rawParams: {} }] }) : final(options)); return `request-${sends}`; } });
		const coordinationEvents: Array<readonly Readonly<{ id: string; generation: number; released: boolean }>[]> = []; const coordinationListener = f.service.onDidChangeRun(event => { if (event.parentId === 'ack-generation') coordinationEvents.push(f.service.getCoordinationRunViews('ack-generation')); });
		...
		assert.deepStrictEqual(f.service.getCoordinationRunViews('ack-generation'), [{ id: first.id, generation: 0, released: false }], 'terminal-but-unreleased child must retain the physical exclusion identity');
		assert.deepStrictEqual(Object.keys(f.service.getCoordinationRunViews('ack-generation')[0]).sort(), ['generation', 'id', 'released'], 'coordination view must expose no transcript, provider, tool, or result data');
		...
		assert.ok(f.service.getRunViews('ack-generation').some(view => view.generation === 1)); assert.deepStrictEqual(f.service.getCoordinationRunViews('ack-generation'), [], 'the successor may complete, but no physically released child may retain the coordination lease');
		assert.ok(coordinationEvents.some(views => !views.some(view => view.id === first.id)), 'post-releaseExecution must publish the physical-release event'); coordinationListener.dispose();
	});

	test('does not yield a parent scheduler slot for a targetless wait with no direct child', async () => {
		let sends = 0; const f = fixture({ send: options => { sends++; queueMicrotask(() => sends === 1 ? options.onFinalMessage({ fullText: 'wait', fullReasoning: '', anthropicReasoning: null, toolCalls: [{ id: 'wait', name: 'wait_agent', rawParams: { timeout_ms: 0 } }] }) : final(options)); return `request-${sends}`; } });
	...
	test('cancels a nested subtree from its direct root parent and fences late child completion', async () => {
		let sends = 0; const pending: any[] = []; const f = fixture({ send: options => { sends++; if (sends === 1) queueMicrotask(() => options.onFinalMessage({ fullText: 'delegate', fullReasoning: '', anthropicReasoning: null, toolCalls: [{ id: 'spawn', name: 'spawn_agent', rawParams: { message: 'nested' } }] })); else pending.push(options); return `request-${sends}`; } });
	...
	test('cancels a deferred nested admission with its parent before it can append or dispatch', async () => {
		...
		const f = fixture({ readSkillBody: async () => { entered(); await deferred; return { body: skillText('demo') }; }, send: options => { sends++; if (sends === 1) queueMicrotask(() => options.onFinalMessage({ fullText: 'delegate', fullReasoning: '', anthropicReasoning: null, toolCalls: [{ id: 'spawn', name: 'spawn_agent', rawParams: { message: '$demo nested' } }] })); return `request-${sends}`; } });
	...
	test('interrupt wins a late final with one terminal event, abort, and invalidation', async () => {
		...
		let releaseSetup!: () => void; const setup = new Promise<void>(resolve => releaseSetup = resolve); let setupStarted = false; let setupInterrupts = 0; let releaseResult!: (value: unknown) => void; const setupResult = new Promise<unknown>(resolve => releaseResult = resolve);
		const setupRace = fixture({ send: toolThenFinal({ id: 'setup-read', name: 'read_file', rawParams: { uri: 'file:///workspace/a' } }) });
		(setupRace.service as any).tools.callTool.read_file = async () => { setupStarted = true; await setup; return { result: setupResult, interruptTool: () => { setupInterrupts++; throw new Error('expected interrupt failure'); } }; };
		const setupChild = await setupRace.service.spawn('setup-race', 'inspect', snapshot()); for (let index = 0; index < 10 && !setupStarted; index++) await Promise.resolve(); assert.strictEqual(setupStarted, true); setupRace.service.interrupt('setup-race', setupChild.id); releaseSetup();
		for (let index = 0; index < 10 && setupInterrupts === 0; index++) await Promise.resolve(); assert.strictEqual(setupInterrupts, 1); assert.strictEqual((setupRace.service as any).groups.get('setup-race').runs[0].activeDirectInterrupts.size, 0);
		setupRace.service.forgetParent('setup-race'); let successorGranted = false; const successorPending = setupRace.service.acquireGroupIo('setup-race', 1, 'write').then(lease => { successorGranted = true; return lease; }); await Promise.resolve(); assert.strictEqual(successorGranted, false);
		releaseResult({ text: 'drained' }); const successor = await successorPending; assert.strictEqual(successorGranted, true); successor.release(); setupRace.service.dispose();
	});
	...
	test('a fresh service has no restart replay and forgetParent removes transient terminal state', async () => {
		...
		const orphanedWrite = await coordinator.service.acquireGroupIo('io-parent', 2, 'write'); coordinator.service.forgetParent('io-parent'); let orphanedParentGranted = false; const orphanedParent = coordinator.service.acquireGroupIo('io-parent', 3, 'read').then(lease => { orphanedParentGranted = true; return lease; }); await Promise.resolve(); assert.strictEqual(orphanedParentGranted, false); assert.strictEqual((coordinator.service as any).groups.has('io-parent'), false);
		orphanedWrite.release(); const orphanedNoop = await orphanedParent; assert.strictEqual(orphanedParentGranted, true); orphanedNoop.release(); assert.strictEqual((coordinator.service as any).groups.has('io-parent'), false);
