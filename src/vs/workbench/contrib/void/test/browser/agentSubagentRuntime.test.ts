...
suite('Void agent subagent Chat runtime routing', () => {
	...
	test('storage normalization and destructive lifecycle keep durable activity inert', async () => {
		...
		const agentEvent = { role: 'agent', sourceId: 'child', kind: 'completion', sequence: 4, status: 'completed', content: 'result', createdAt: 1 };
		const withAgent = (ChatThreadService.prototype as any)._convertThreadDataFromStorage.call(service, JSON.stringify({ parent: { id: 'parent', messages: [{ role: 'user', content: 'question', displayContent: 'question' }, agentEvent], state: {}, filesWithUserChanges: [] } }));
		assert.deepStrictEqual(withAgent.parent.messages[1], agentEvent); assert.strictEqual(withAgent.parent.messages.filter((message: any) => message.role === 'user').length, 1);
	...
	test('routes controls before builtin/MCP and preserves the parent tool id', async () => {
		...
		await runTool(fixture.value, 'spawn_agent', 'background-tool-id', { message: 'independent', background: true }); assert.strictEqual(fixture.calls[1][1][14], true);
	});
	...
	test('terminal child events do not restart an idle parent provider turn', () => {
		assert.strictEqual((ChatThreadService.prototype as any)._scheduleAgentMailboxContinuation, undefined);
	});
