...
suite('Void agent subagent Chat runtime routing', () => {
	...
	test('routes controls before builtin/MCP and preserves the parent tool id', async () => { const fixture = receiver(); await runTool(fixture.value, 'spawn_agent', 'provider-tool-id', { message: 'inspect' }); assert.strictEqual(fixture.calls[0][0], 'spawn'); assert.strictEqual(fixture.calls[0][1][3], undefined); assert.strictEqual(fixture.messages.length, 1); assert.strictEqual(fixture.messages[0].id, 'provider-tool-id'); assert.strictEqual(fixture.messages[0].mcpServerName, undefined); });
	test('fails closed without exact current authority and rejects unknown fields before state', async () => {
		...
		assert.deepStrictEqual(fixture.calls, []); assert.ok(/agent_delegation_not_authorized/.test(fixture.messages.pop().content));
		await runTool(fixture.value, 'spawn_agent', 'bad-id', { message: 'inspect', model: 'override' });
	...
	test('central revoke is one-shot for purge, delete, replacement, reset, and dispose; switching tasks is non-destructive', () => {
		...
		const deleted = make({ parent: { state: {} } }); deleted.value._cancellingToolReceiptsOfThread.set('parent', new Map()); deleted.value._activeToolCardReceiptsOfThread.set('parent', new Map()); ChatThreadService.prototype.deleteThread.call(deleted.value, 'parent'); assert.deepStrictEqual(deleted.forgotten, ['parent']); assert.strictEqual(deleted.value._agentControlGeneration.has('parent'), false); assert.strictEqual(deleted.value._cancellingToolReceiptsOfThread.has('parent'), false); assert.strictEqual(deleted.value._activeToolCardReceiptsOfThread.has('parent'), false);
		const replaced = make({ parent: { state: {} }, other: { state: {} } }); ChatThreadService.prototype.dangerousSetState.call(replaced.value, { allThreads: {}, currentThreadId: 'replacement' }); assert.deepStrictEqual(replaced.forgotten.sort(), ['other', 'parent']); assert.strictEqual(replaced.value._agentControlGeneration.size, 0);
		const reset = make({ parent: { state: {} }, other: { state: {} } }); ChatThreadService.prototype.resetState.call(reset.value); assert.deepStrictEqual(reset.forgotten.sort(), ['other', 'parent']); assert.strictEqual(reset.value._agentControlGeneration.size, 0);
	...
	test('a named Agent role is exact parent intent, never an automatic child', async () => {
		...
		const routed = receiver(); const authority = Object.freeze({ allowed: true, generation: 0, roles: { revision: 'catalog-1', agents: [role], diagnostics: [] }, settingsState: {}, settingsOfProvider: {} });
		...
		assert.strictEqual(routed.calls[0][0], 'spawn'); assert.strictEqual(routed.calls[0][1][3], 'project-reader');
	});
