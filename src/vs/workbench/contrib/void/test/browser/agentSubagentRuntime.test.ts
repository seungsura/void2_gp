...
const receiver = () => { const messages: any[] = []; const calls: any[] = []; return { messages, calls, value: hydrateLifecycle({ state: { allThreads: { parent: {} }, currentThreadId: 'parent' }, streamState: {}, _agentInstructionSessionOfThread: new Map(), _instructionTurnOfThread: new Map(), _agentControlGeneration: new Map(), _agentDelegationAuthorityOfThread: new Map(), _transientComposerDraftOfThread: new Map(), _revokeAgentDelegation: revoke, _agentCustomAgentService: { getCatalog: async () => ({ revision: 'roles', agents: [], diagnostics: [] }) }, _wakeAgentWaitForPendingSteer: () => false, _agentSubagentService: { spawn: async (...args: any[]) => { calls.push(['spawn', args]); return { id: 'child', status: 'queued' }; }, wait: async () => { calls.push(['wait']); return { id: 'child', status: 'running', deliverSummary: false }; }, interrupt: () => { calls.push(['interrupt']); return { id: 'child', status: 'cancelled' }; }, forgetParent: (id: string) => calls.push([`forget:${id}`]) }, _toolsService: { invalidateReadReceipts(_threadId: string) { }, validateParams: new Proxy({}, { get() { throw new Error('builtin lookup'); } }) }, _mcpService: { getMCPTools() { throw new Error('MCP lookup'); } }, _addMessageToThread: (_: string, message: any) => messages.push(message), openNewThread() { }, _onDidChangeCurrentThread: { fire() { } } }) }; };
...
suite('Void agent subagent Chat runtime routing', () => {
	...
	test('generation drift and reset during deferred wait drop every late tool result', async () => {
		const fixture = receiver(); let release!: () => void; let steerChecks = 0; fixture.value._wakeAgentWaitForPendingSteer = () => { steerChecks++; return false; }; fixture.value._agentSubagentService.wait = async () => { fixture.calls.push('wait'); await new Promise<void>(resolve => release = resolve); return { id: 'child', status: 'completed', deliverSummary: true }; }; const pending = runTool(fixture.value, 'wait_agent', 'wait-id', { timeout_ms: 30000 }); await Promise.resolve(); assert.strictEqual(steerChecks, 1); fixture.value._agentControlGeneration.set('parent', 1); release(); assert.deepStrictEqual(await pending, { interrupted: true }); assert.deepStrictEqual(fixture.messages, []);
	...
	test('late completion reopens the broker history run before provider validation and a newer generation fences it', async () => {
		const build = (generation: number, invalidateDuringSync = false) => { const messages: any[] = []; let acked = 0, synced = 0, authorized = 0, verified = 0, validated = 0, started = 0, providerRequests = 0; let historyRunOpen = false, trusted = true; const runtimeSnapshot: any = { ownerProjectRoot: 'file:///workspace', workspaceTrustedAtAdmission: true, model: { hasModel: true, providerName: 'openAI', modelName: 'gpt-4.1', modelSelectionOptions: {} } }; const authority = { allowed: true, generation: 1, runtimeSnapshot }; const broker = {
			...
			_agentMailboxContinuationScheduled: new Set(), _runQuiescenceOfThread: new Map(), _agentControlGeneration: new Map([['parent', generation]]), _parentRunTokenOfThread: new Map(), _startingParentRunOfThread: new Map(), _pendingChatSubmissionOfThread: new Map(), _childGroupSourceOfThread: new Map([['parent', { runId: 'run', generation: 1 }]]), _agentDelegationAuthorityOfThread: new Map([['parent', authority]]), state: { allThreads: { parent: { messages } } },
			_workspaceContextService: { getWorkspace: () => ({ folders: [{ uri: URI.parse('file:///workspace') }] }) }, _workspaceTrustManagementService: { isWorkspaceTrusted: () => trusted }, _isAwaitingUser: () => false,
			_syncActiveChildGroup: async () => { await Promise.resolve(); synced++; if (invalidateDuringSync) { trusted = false; value._agentDelegationAuthorityOfThread.set('parent', { ...authority }); } return { ok: true, identity: { sourceRevision: 2, generation: 1, childIds: ['other-active-child'] } }; },
		...
		const invalidated = build(1, true); (ChatThreadService.prototype as any)._scheduleAgentMailboxContinuation.call(invalidated.value, 'parent', 1, { runId: 'run', generation: 1 }); await new Promise(resolve => setTimeout(resolve, 10)); assert.strictEqual(invalidated.messages.length, 0); assert.deepStrictEqual(invalidated.counts(), { acked: 0, synced: 1, authorized: 0, verified: 0, validated: 0, started: 0, providerRequests: 0 });
	});
