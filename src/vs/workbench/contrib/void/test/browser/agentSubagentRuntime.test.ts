...
suite('Void agent subagent Chat runtime routing', () => {
	...
	test('late completion resumes once after durable mailbox append and a newer generation fences it', async () => {
		const build = (generation: number) => { const messages: any[] = []; let acked = 0; let started = 0; const runtimeSnapshot: any = { ownerProjectRoot: 'file:///workspace', workspaceTrustedAtAdmission: true, model: { hasModel: true, providerName: 'openAI', modelName: 'gpt-4.1', modelSelectionOptions: {} } }; const value: any = {
			...
			_startTrackedParentRun: (_id: string, _run: any, start: () => Promise<void>) => { started++; void start(); }, _runChatAgent: async () => { },
		}; return { value, messages, counts: () => ({ acked, started }) }; };
		const accepted = build(1); (ChatThreadService.prototype as any)._scheduleAgentMailboxContinuation.call(accepted.value, 'parent', 1, { runId: 'run', generation: 1 }); await new Promise(resolve => setTimeout(resolve, 5)); assert.strictEqual(accepted.messages.length, 1); assert.deepStrictEqual(accepted.counts(), { acked: 1, started: 1 });
		const stopped = build(2); (ChatThreadService.prototype as any)._scheduleAgentMailboxContinuation.call(stopped.value, 'parent', 1, { runId: 'run', generation: 1 }); await new Promise(resolve => setTimeout(resolve, 5)); assert.strictEqual(stopped.messages.length, 0); assert.deepStrictEqual(stopped.counts(), { acked: 0, started: 0 });
	});
