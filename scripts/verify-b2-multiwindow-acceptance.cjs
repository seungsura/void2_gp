...
async function main() {
	...
	try {
		...
		stage = 'resume durable W4 input once'; await resumed.click(); await eventually(() => fakeServer.requestSummaries().filter(summary => summary.userText.includes(queuedText)).length === 1, 'W4 Resume did not send the durable queue exactly once'); await waitVisible(third.page.getByText('Void smoke fixture completed.', { exact: true }).last(), 'W4 resumed fake response'); await third.page.locator('#void-chat-current-stop:visible').waitFor({ state: 'hidden', timeout: timeoutMs }); await resumedQueued.waitFor({ state: 'hidden', timeout: timeoutMs }); console.log('B2 stage: W4 resume delivered once and settled');
