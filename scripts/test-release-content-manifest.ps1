...
try {
    ...
    $packageScriptPath=Join-Path $PSScriptRoot 'package-portable.ps1';$packageTokens=$null;$packageParseErrors=$null;$null=[System.Management.Automation.Language.Parser]::ParseFile($packageScriptPath,[ref]$packageTokens,[ref]$packageParseErrors);if($packageParseErrors.Count -ne 0){throw "package-portable parser errors: $($packageParseErrors.Message -join '; ')"};$packageScriptText=[IO.File]::ReadAllText($packageScriptPath);if($packageScriptText.Contains('$PSDefaultParameterValues')){throw 'package-portable must not use script-global archive validation defaults.'};$portableArchiveCalls=@($packageScriptText -split "`r?`n"|Where-Object{$_.Contains('Assert-PortableArchive -ArchivePath')});if($portableArchiveCalls.Count -ne 6 -or @($portableArchiveCalls|Where-Object{-not $_.Contains('-ArchiveMode')}).Count -ne 0){throw 'Every package-portable archive validation call must select an explicit archive mode.'};foreach($marker in @("[ValidateSet('Current', 'Historical')][string]`$ArchiveMode = 'Current'", "-HistoricalArchive:(`$ArchiveMode -ceq 'Historical')", "[ValidateSet('Current', 'Historical')][string]`$CandidateArchiveMode = 'Current'", "`$PreparedArchiveMode = if (`$HistoricalArchive) { 'Historical' } else { 'Current' }", "Assert-PortableArchive -ArchivePath `$TemporaryPath -PayloadEntries `$PayloadEntries -ContentManifest `$ContentManifest -RequiredContract `$CandidateRequiredDocsContract -ArchiveMode `$CandidateArchiveMode", "Assert-PortableArchive -ArchivePath `$OutputPath -PayloadEntries `$PayloadEntries -ContentManifest `$ContentManifest -ArchiveMode Historical", "Assert-PortableArchive -ArchivePath `$OutputPath -PayloadEntries `$PayloadEntries -ContentManifest `$ContentManifest -RequiredContract `$previousDocsContract -ArchiveMode Historical", "Assert-PortableArchive -ArchivePath `$PreparedArchivePath -PayloadEntries `$RuntimePayloadEntries -ContentManifest `$ReleaseContentManifest -RequiredContract `$RequiredDocsContract -ArchiveMode `$PreparedArchiveMode", "Assert-PortableArchive -ArchivePath `$TemporaryOutputPath -PayloadEntries `$RuntimePayloadEntries -ContentManifest `$ReleaseContentManifest -RequiredContract `$CandidateRequiredDocsContract -ArchiveMode `$CandidateArchiveMode", "-CandidateArchiveMode `$CandidateArchiveMode")){if(-not $packageScriptText.Contains($marker)){throw "package-portable archive mode routing is missing: $marker"}};Add-Pass 'portable archive mode parser and routing'
    $releaseScriptText=[IO.File]::ReadAllText((Join-Path $PSScriptRoot 'release-win32-x64-portable.ps1'));foreach($marker in @('param([string]$Path,[int]$RequiredDocsContract=-1,[switch]$HistoricalArchive)','param([string]$Path,[AllowNull()][psobject]$ExpectedPortable=$null,[switch]$HistoricalArchive)',"`$historicalArchive=`$TransactionTag -cne 'forward'",'Assert-PortableArchive $portableFiles[0].FullName -HistoricalArchive','Assert-OuterArchive $outerFiles[0].FullName $portable -HistoricalArchive','Assert-OuterArchive $outerCopy $portable -HistoricalArchive','Assert-OuterArchive $outerPath $portable -HistoricalArchive','Assert-CanonicalPackageFiles $context.oldValidated.portable $context.oldValidated.outer $context.stage -HistoricalArchive','Assert-PortableArchive $candidatePath -RequiredDocsContract 1')){if(-not $releaseScriptText.Contains($marker)){throw "Historical/strict archive routing is missing: $marker"}};Add-Pass 'historical preflight and strict candidate routing'
    ...
    $mismatchZip=Join-Path $positive.Root 'mismatch.zip';New-DocsFixtureZip $mismatchZip $positiveManifest mismatch;Assert-Throws 'current prepared candidate rejects old release-notes' {Test-DocsFixtureZip $mismatchZip $positiveManifest 1|Out-Null};$historicalMismatch=Test-DocsFixtureZip $mismatchZip $positiveManifest -HistoricalArchive;if($historicalMismatch.Contract -ne 1 -or $historicalMismatch.Count -ne 3){throw 'Historical release-notes fixture did not return contract metadata.'};$historicalNotes=@($historicalMismatch.Entries|Where-Object{$_.Path -ceq 'docs/release-notes.md'})[0];if($historicalNotes.Length -le 0 -or [string]::IsNullOrWhiteSpace($historicalNotes.Sha256)){throw 'Historical release-notes fixture did not return actual metadata.'};$currentPreparedZip=Join-Path $positive.Root 'current-prepared.zip';New-DocsFixtureZip $currentPreparedZip $positiveManifest exact;$currentPrepared=Test-DocsFixtureZip $currentPreparedZip $positiveManifest 1;if($currentPrepared.Contract -ne 1 -or $currentPrepared.Count -ne 3){throw 'Current prepared candidate did not retain strict docs contract metadata.'};Add-Pass 'historical final replaced by strict current prepared candidate'
