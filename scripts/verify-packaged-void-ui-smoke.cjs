...
async function runFakeAcceptance(page, evidence, fakeServer) {
	...
	const pending = page.locator('section[aria-label="Queued messages"]'); await waitVisible(pending, 'Queued messages'); await waitVisible(pending.getByText('VOID_SMOKE_QUEUE', { exact: true }), 'queued fixture message'); await waitVisible(pending.getByText('VOID_SMOKE_STEER', { exact: true }), 'steered fixture message'); const pendingText = await pending.innerText(); if (!pendingText.includes('Queue') || !pendingText.includes('Steer')) throw new Error('Queue and Steer were not retained while terminal was live.'); evidence.assertions.push('queue-and-steer-during-live-tool');
