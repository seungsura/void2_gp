...
async function runFakeAcceptance(page, evidence, fakeServer) {
	...
	await chat.fill('VOID_SMOKE_STEER'); await page.getByRole('combobox', { name: 'More message actions', exact: true }).selectOption('steer'); await waitVisible(pending.getByText('VOID_SMOKE_STEER', { exact: true }), 'steered fixture message'); const pendingText = await pending.innerText(); if (!pendingText.includes('Queue') || !pendingText.includes('Steer')) { const terminalStopState = { visible: await terminalStop.isVisible(), disabled: await terminalStop.isDisabled(), currentStopVisible: await page.locator('#void-chat-current-stop').isVisible().catch(() => false) }; throw new Error(`Queue and Steer were not retained while terminal was live: ${pendingText.slice(0, 500)}; terminalStop=${JSON.stringify(terminalStopState)}`); } evidence.assertions.push('queue-and-steer-during-live-tool');
