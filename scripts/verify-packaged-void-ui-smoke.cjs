...
async function runFakeAcceptance(page, evidence, fakeServer) {
	...
	await chat.fill('VOID_SMOKE_STEER'); await page.getByRole('combobox', { name: 'More message actions', exact: true }).selectOption('steer'); await waitVisible(pending.getByText('VOID_SMOKE_STEER', { exact: true }), 'steered fixture message'); const pendingText = await pending.innerText(); if (!pendingText.includes('Queue') || !pendingText.includes('Steer')) throw new Error('Queue and Steer were not retained while terminal was live.'); evidence.assertions.push('queue-and-steer-during-live-tool');
