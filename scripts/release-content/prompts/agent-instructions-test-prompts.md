...
Expected: supported native Agent route는 marker 없이 generic controls와 discovered named role을 제공하고 parent의 `spawn_agent` call만 child를 시작합니다. Marker 없는 fresh turn에서도 parent가 exact `reviewer` `agent_type`으로 named child를 명시적으로 시작할 수 있으며, named picker selection을 사용한 turn에서도 `agent_type`은 exact `reviewer`입니다. Spawn receipt 뒤 durable Child Activity card에는 role name, `read_only`와 short identity가 표시됩니다. 기본 spawn은 async로 즉시 반환하지만 child terminal 전 premature parent final은 표시되지 않고, attributed Agent result를 반영한 final이 한 번 나타납니다. Child가 같은 본문을 `send_message`한 뒤 final로 반환해도 본문은 한 번만 보이며 terminal status가 연결됩니다. 독립 작업은 `background=true`로 시작하고 이 completion은 idle parent provider를 재시작하지 않습니다. `spawn_agent` UI title은 **Start child Agent**이며 generic approval 또는 MCP card가 아닙니다.
