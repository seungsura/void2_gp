...
Spawn receipt에 묶인 bounded expandable Child activity card는 볼 수 있지만 full child transcript/session은 아닙니다. Child/group 전체 wall-clock·turn·cumulative-send quota는 없고 configured scheduler capacity와 operation timeout, 실제 logical in-flight provider-dispatch lease만 적용됩니다. Provider-native multi-tool batch는 ordinal identity를 보존해 provider order로 serial 실행하며 literal `multi_tool_use.parallel`은 없습니다. exact safe read만 최대 2개 concurrent이고 mutation/terminal/MCP는 serialized/exclusive입니다. Plan/orchestration은 main parent가 소유하며 새 Plan API/UI/storage나 same-child follow-up은 제공하지 않습니다.
