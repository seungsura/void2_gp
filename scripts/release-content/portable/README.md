...
`$`와 `@`는 같은 Skills catalog를 사용하고 application tools는 MCP가 아닌 exact cards로 표시됩니다. Running tool card는 elapsed와 exact card Stop 상태를 표시하며 Queue/Steer는 안전한 boundary까지 pending 상태를 보존합니다. Landing에는 non-empty Chat history가 있지만 current composer 아래에는 persistent list가 없으며 `View Past Chats`로 돌아갑니다. Exact `(empty message)`는 저장·표시·outbound history에 남지 않고 non-empty reasoning-only UI는 유지됩니다.
