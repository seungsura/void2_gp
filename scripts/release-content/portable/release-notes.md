...
- `inherit_parent_write` role은 frozen parent tool snapshot을 broker로 사용합니다. Frozen tools와 Undo availability는 durable Child Activity card에서 보며, captured parent approval policy가 적용되는 pending manual approval은 composer-adjacent Approve/Reject card에서 처리합니다. One mutation-capable child만 동시에 실행되며 nested child도 같은 group budget/cancellation을 공유하고 live authority를 얻지 않습니다.
