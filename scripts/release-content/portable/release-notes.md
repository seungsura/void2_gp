...
- Spawn별 optional model/reasoning/context fork, bounded same-root tree/result inspection, active parent/peer/child queue-only messaging과 targeted interrupt를 제공합니다. Parent는 모든 child를 강제로 기다리지 않으며 late completion은 Stop/new turn/owner/generation fence 뒤 coalesced continuation으로 전달됩니다.
